...
public class WMS {
	...
	// 2D list where each element is itself a list representing a path
	private List<Integer>[][] allMaps;
	...
	private void computePathsForGrid() {
		int gridSize = graph.getGridWidth() * graph.getGridHeight();
		allMaps = new ArrayList[gridSize][gridSize];

		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize; j++) {
				if (i == j) {
					allMaps[i][j] = new ArrayList<>(Arrays.asList(i)); // Path from a node to itself.
				} else {
					allMaps[i][j] = graph.dijkstra(i, j); // Compute the shortest path from i to j.
				}
			}
		}
	}
